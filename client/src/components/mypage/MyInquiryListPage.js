...
const Lock = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden>
    <path
      d="M17 8h-1V6a4 4 0 10-8 0v2H7a2 2 0 00-2 2v8a2 2 0 002 2h10a2 2 0 002-2v-8a2 2 0 00-2-2zm-7-2a2 2 0 114 0v2H10V6zm7 12H7v-8h10v8z"
      fill="#7b7b7b"
    />
  </svg>
);
...
export default function MyInquiryListPage() {
  ...
  const [selectedCat, setSelectedCat] = useState("전체"); // 선택된 카테고리
  ...
  const filtered = useMemo(() => {
    ...
    return ROWS.filter((r) => {
      ...
      const qOk =
        !q || r.title.includes(q) || r.category.includes(q) || r.status.includes(q);
      return catOk && qOk;
    });
  }, [selectedCat, query]);
    ...
    <div style={styles.page}>
      ...
      <div style={styles.topBar}>
        <div style={{ position: "relative" }}>
          <button
            ...
            aria-expanded={open}
          >
            <span style={{ color: "#333" }}>{selectedCat === "전체" ? "전체" : selectedCat}</span>
            ...
            <div ref={panelRef} style={styles.dropdownPanel}>
              {CATEGORIES.map((c) => (
                <div
                  key={c}
                  onClick={() => {
                    setSelectedCat(c === "전체" ? "전체" : c);
                    setOpen(false);
                  }}
                  style={{
                    ...styles.dropdownItem,
                    ...(selectedCat === c ? styles.dropdownItemActive : {}),
                  }}
                >
                  {c}
                </div>
      ...
      <div style={styles.tableWrap}>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((r) => (
                <tr key={r.id}>
                  ...
                  <td
                    style={{
                      color: r.status === "답변 완료" ? "#76a36a" : "#666",
                      fontWeight: r.status === "답변 완료" ? 800 : 600,
                      textDecoration: r.status === "답변 완료" ? "underline" : "none",
                      textUnderlineOffset: r.status === "답변 완료" ? 3 : 0,
                    }}
                  >
                    {r.status}
                  </td>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 20 }}>
          <button
            type="button"
            onClick={() => (window.location.href = "/mypage/inquiry/new")}
            style={styles.askBtn}
          >
            문의하기
          </button>
...
`;
if (typeof document !== "undefined" && !document.getElementById("inquiry-list-inline-style")) {
  const tag = document.createElement("style");
  tag.id = "inquiry-list-inline-style";
